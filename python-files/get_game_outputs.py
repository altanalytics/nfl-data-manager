...
def get_game_outputs(unique_game_id: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        game_outputs["_metadata"] = {
            "s3_path": f"s3://{s3_bucket}/{s3_path}",
            "unique_game_id": unique_game_id,
            "season": season,
            "season_type": season_type,
            "week": week,
            "files_found": len([k for k in game_outputs.keys() if k != "_metadata"])
        }
...
def main():
    ...
    print(f"=== GETTING OUTPUT FILES FOR GAME: {game_id} ===\n")
    
    outputs = get_game_outputs(game_id)
    ...
    # Print metadata
    if "_metadata" in outputs:
        metadata = outputs["_metadata"]
        print("METADATA:")
    ...
    files = [k for k in outputs.keys() if k != "_metadata"]
    print("FILES FOUND:")
    for filename in files:
        file_data = outputs[filename]
    ...
    if files:
        ...
        sample_data = outputs[first_file]
